...
public class UserServiceImpl implements IUserService {
    ...
    public BSResult login(String username, String password) {
        ...
        if (users == null || users.size() == 0) {
            return BSResultUtil.build(400, "用户不存在");
        }
        ...
        if (!DigestUtils.md5DigestAsHex(password.getBytes()).equals(user.getPassword())) {
            return BSResultUtil.build(400, "密码错误");
        }
    ...
    public BSResult saveUser(User user) {
        ...
        user.setActive(userResourceProperties.getIsNotActive());//默认未激活
